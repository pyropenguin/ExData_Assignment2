...
emissionsSumm <- emissions[fips == "24510",
                           list(totalPM25Emissions = sum(Emissions)),
                           by=list(type,year)]
setkey(emissionsSumm, year)
png('./plot3.png', width = 480, height = 480)
  g <- ggplot(data=emissionsSumm, aes(x=year, y=totalPM25Emissions, fill=type))
  g <- g + geom_bar(stat='identity')
  g <- g + facet_grid(. ~ type)
  g <- g + labs(title='Total PM2.5 emission from Baltimore City, Maryland by type',
