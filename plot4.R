...
# NOTE: SCC.Level.Three has the fuel source data. Filter by "coal".
emissionsSumm <- emissions[grep("[Cc]oal", emissions$SCC.Level.Three),
                           list(totalPM25Emissions = sum(Emissions)),
                           by=list(year,SCC.Level.Three)]
setkey(emissionsSumm, year)
png('./plot4.png', width = 1080, height = 720)
  g <- ggplot(data=emissionsSumm, aes(x=year, y=totalPM25Emissions, fill=SCC.Level.Three))
  g <- g + geom_bar(stat='identity')
  g <- g + facet_wrap(~ SCC.Level.Three, nrow=3, ncol=3)
  g <- g + labs(title='Total PM2.5 emission from Coal-related sources',
