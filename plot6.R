...
# fips == 24510 is Baltimore City, 06037 is Los Angeles County.
emissionsSumm <- emissionsSumm[fips %in% c("24510", "06037")] 
levels(emissionsSumm$fips)[levels(emissionsSumm$fips)=="24510"] <- "Baltimore City"
levels(emissionsSumm$fips)[levels(emissionsSumm$fips)=="06037"] <- "Los Angeles County"
setkey(emissionsSumm, year)
png('./plot6.png', width = 1200, height = 720)
  g <- ggplot(data=emissionsSumm, aes(x=year, y=totalPM25Emissions, fill=SCC.Level.Two))
  g <- g + geom_bar(stat='identity')
  g <- g + facet_grid(fips ~ SCC.Level.Two)
  g <- g + labs(title='Total PM2.5 emission from motor vehicles in Baltimore City and Los Angeles County',
